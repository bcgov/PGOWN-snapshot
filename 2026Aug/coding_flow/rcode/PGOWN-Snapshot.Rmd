...
if (is.null(params$yaml_file) || !nzchar(params$yaml_file)) {
  stop("The yaml_file parameter was not supplied to the R Markdown report.")
}

if (!file.exists(params$yaml_file)) {
  stop(
    paste0(
      "The YAML parameter file does not exist: ",
      params$yaml_file
    )
  )
}

parameters <- yaml::read_yaml(params$yaml_file)
...
timeseries_client_file <- file.path(
  here::here(),
  report_folder,
  "coding_flow",
  "rcode",
...
message("Timeseries client path: ", timeseries_client_file)
message(
  "Timeseries client exists: ",
  file.exists(timeseries_client_file)
)

if (!file.exists(timeseries_client_file)) {
  stop(
    paste0(
      "Cannot find timeseries_client.R at: ",
      timeseries_client_file
    )
  )
}
...
write.csv(all_approval_dat, file = file.path(
  here::here(),
  report_folder,
  "coding_flow",
  "data",
...
number_of_active_wells <- read.csv(file.path(
  here::here(),
  report_folder,
  "coding_flow",
  "data",
...
ggsave(file.path(here::here(), report_folder, "coding_flow", "generated", "figures", "Fig1_NumberOfWells.png"), p1, width = 7, height = 4, units = "in")


write.csv(number_of_active_wells, file = file.path(here::here(), report_folder, "coding_flow", "generated", "figures", "Fig1_NumberOfWells.csv"), row.names = FALSE)

write.csv(number_of_active_wells, file = file.path(here::here(), report_folder, "coding_flow", "data", "data_new", "NumberOfWells.csv"), row.names = FALSE)
...
write_csv(df_loc_to_exp %>% select(Identifier, DataSource), 
          file.path(here::here(), report_folder, "coding_flow", "data", "data_rc", "ow_telem.csv"))
...
write_csv(df_grade %>% left_join(well_lookup, by = c("ObsWell" = "Well_Name")), 
          file.path(here::here(), report_folder, "coding_flow", "data", "data_rc", "PGOWN_Grades_Appr.csv"))
...
df_gap <- df_recent_year %>% 
  ...
  filter(Timestamp >= as.POSIXct(date_data_gap_start) & Timestamp <= as.POSIXct(date_data_gap_end)) %>% 
  group_by(ObsWell) %>% 
...
#obswell_list     <- unique(well_lookup$Well_Name)
  ...
  df_gap_tmp <- df_gap %>% 
    ...
    mutate(
      gap = case_when(
        ...
        #min(Timestamp) >= as.POSIXct(date_data_gap_start) & max(Timestamp) <= as.POSIXct(date_data_gap_end) ~ (max(Timestamp) - min(Timestamp)),
        ...
        is_last & max(Timestamp) < as.POSIXct(date_data_gap_end) & global_min <= as.POSIXct(date_data_gap_start) ~ (as.POSIXct(date_data_gap_end) - (max(Timestamp))),
        TRUE                                     ~ gap
      )
    )
...
df_gap_complete %>% 
  ...
  group_by(ObsWell, first_obs) %>% 
  summarize(GapHours = sum(GapHours, na.rm = TRUE)) %>% 
  #ungroup() %>% 
  #unique() %>%
  left_join(well_lookup, by = c("ObsWell" = "Well_Name")) %>% 
  mutate(GapHours = ifelse(first_obs > date_data_gap_start, NA, GapHours)) %>%
...
data_gaps <- merge(data_gaps, well_lookup, by.x = "ObsWell", by.y = "Well_Name", all.x = F, all.y = T) %>% dplyr::select(-Region.x) %>% rename(Region = Region.y)  %>% 
  #added to filter new wells from analysis on data gaps
  dplyr::filter(first_obs < date_data_gap_end)
...
##number of well per region
#Cannot use well lookup since it will also count new wells
#count_well_by_region <- well_lookup %>% group_by(Region) %>% summarize(number_of_wells = n())
count_well_by_region <- data_gaps %>% dplyr::select(ObsWell, Region) %>% group_by(Region) %>% summarize(number_of_wells = n())
...
total_prov_data_gaps_wells <- data_gaps_summary %>% dplyr::select(no.wells) %>% sum()
...
prov_data_gaps_summary <- data_gaps_summary %>% group_by(gap_type) %>% summarize(Region = "Province-wide", 
                                                            ...
                                                            well.per.reg = total_prov_data_gaps_wells, percent = (no.wells/well.per.reg)*100)
